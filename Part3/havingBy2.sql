...
select state , count(state) as calisan_sayisi from workers group by state having count(state)>1
...
select company , min(salary) from workers group by company having min(salary)>2000

--Tekrarsiz isimleri cağırın
